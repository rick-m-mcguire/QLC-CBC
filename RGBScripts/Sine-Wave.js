			...
			for (var y = 0; y < height; y++)
			{
				...
				for (var x = 0; x < width; x++)
				{
					switch (algo.Orientation) {
						case "Horizontal":
						map[y][x] = HSVToQRgb(h,s,v*((algo.IntensityMax-algo.IntensityMin)/100*(Math.sin((step+x)*2*Math.PI/width)+1)/2+algo.IntensityMin/100));
						break;
						case "Vertical":
						map[y][x] = HSVToQRgb(h,s,v*((algo.IntensityMax-algo.IntensityMin)/100*(Math.sin((step+y)*2*Math.PI/height)+1)/2+algo.IntensityMin/100));
						break;
					}
				}
			}
